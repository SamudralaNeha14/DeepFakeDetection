...
class DeepfakeDetector:
    ...
    def process_video(self, file_data):
        ...
        # Count the frames and faces detected
        total_frames = 0
        total_faces = 0
        ...
        # Calculate face detection ratio
        face_detection_ratio = total_faces / total_frames if total_frames > 0 else 0

        return None, is_real, face_detection_ratio
...
def main():
    ...
    if detection_mode == "Image":
        ...
        if video_file is not None:
            if st.button("Detect"):
                _, is_real, face_detection_ratio = detector.detect_deepfake(video_file, 'video')
                if is_real and face_detection_ratio > 0.5:
